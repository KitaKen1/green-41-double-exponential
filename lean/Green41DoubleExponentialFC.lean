...
/-!
# Green's Problem 41: the double-exponential target

This file proves the `research open` theorem `Green41.green_41.variants.double_exponential_bound`
of Formal Conjectures with the answer `True`: for some `C` and `ε₀ > 0`,
`minCopies ε ≤ exp (exp (ε ^ (-C)))` for all `0 < ε ≤ ε₀`. The definitions `pyjamaSet`,
`coveringCopies` and `minCopies` are the ones imported from Formal Conjectures. The exact-value
theorem `green_41` and the polynomial bound are not claimed here.

The development lives in the namespace `Green41Proof`.
-/

namespace Green41Proof

open Green41 Complex Set Pointwise

/-- The right-hand side of `Green41.green_41.variants.double_exponential_bound`. -/
...
theorem discrete_core {p ℓ S H B : ℕ} (hp : 5 ≤ p) (hS : S = (p - 1) * p ^ (ℓ - 1)) (hℓ : 1 ≤ ℓ)
  ...
  have hlow : ∀ a ∈ range H, ∀ a' ∈ range H,
    ...
    · subst haa
      ...
      rw [hre]; linarith
    · simp only [haa, if_false, zero_sub]
  ...
  have hinner : ∀ a ∈ range H, ∑ a' ∈ range H, ((if a = a' then (S : ℝ) * Y.card else 0) - c) =
      S * Y.card - H * c := by
    intro a ha
    rw [Finset.sum_sub_distrib, Finset.sum_ite_eq, if_pos ha, Finset.sum_const, card_range,
      nsmul_eq_mul]
...
theorem kl57With_100 : KL57With 100 := by
  ...
  have hq : q ≠ 0 := by
    rintro rfl
    simp only [size5, size13, if_true] at hlow
    linarith
...
/-- Exact Formal Conjectures target `Green41.green_41.variants.double_exponential_bound`,
with the answer `True`. -/
theorem green_41_double_exponential_bound_solved : answer(True) ↔
    ∃ C : ℝ, ∃ ε₀ > 0, ∀ ε ∈ Ioc 0 ε₀, (minCopies ε : ℝ) ≤ Real.exp (Real.exp (ε ^ (-C))) :=
  ⟨fun _ => doubleExponentialBound, fun _ => trivial⟩

#print axioms green_41_double_exponential_bound_solved
