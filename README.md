...
**Notation.** For $`x\inℝ`$ write $`\|x\|`$ for the distance from $`x`$ to $`ℤ`$. For
$`|u|=1`$ put

```math
S_u(\varepsilon)=\{z\inℂ:\ \|\mathrm{Re}(uz)\|\lt \varepsilon\}.
```

Since $`S_u(\varepsilon)\subseteq \bar u\,E(\varepsilon)`$, it suffices to find a finite set $`\Omega`$ of
unit complex numbers with $`\bigcup_{u\in\Omega}S_u(\varepsilon)=ℂ`$; then
...
Throughout, $`c=100`$, $`\eta=5^{-\varepsilon^{-c}}`$ and $`\delta=13^{-c\,\eta^{-5}}`$. For $`q\inℤ[i]`$ let
...
> $`r=\rho(s_1,t_1)-\rho(s_2,t_2)`$. Let $`w\inℂ`$ and $`q\inℤ[i]`$ satisfy
> ```math
...
an elementary proof, which does not use the solenoid $`\widehat{ℤ[i][1/\bar P,1/\bar Q]}`$,
...
For $`w\inℂ`$, classify the $`(K+1)^2`$ points $`\rho(s,t)w`$ ($`s,t\le K`$) by two data: the
...
class. Then $`q=[x]-[y]`$ lies in $`Mℤ[i]\subseteq\bar P^{\,u}\bar Q^{\,v}ℤ[i]`$, and
...
over all pairs of distinct labels. Then $`L/r\inℤ[i]`$ for every nonzero orbit difference
$`r`$, and $`|r|\ge65^{-K/2}`$.
...
> $`s,t\le T`$, then $`w`$ lies within distance $`H`$ of the lattice $`(D/L)\,ℤ[i]`$.
...
$`|q|_5+|q|_{13}<13^{-h}`$, that is, $`D\mid q`$. Then $`q/r=(L/r)\,q/L\in(D/L)ℤ[i]`$ and
...
> $`a/\bar a`$ ($`a\inℤ[i]`$) with the following properties:
>
...
> $`\{S_\theta(\varepsilon):\theta\in\Theta\}`$ lies within $`H`$ of $`(D/L)ℤ[i]`$. Let $`U`$ be
> as in Lemma C with $`R=2LH`$, and assume $`|D|>2LH\cdot\max|a|`$. Put
> $`\nu=\frac1{2L}+i\sqrt{1-\frac1{4L^2}}`$, so that $`|\nu|=1`$ and $`\nu+\bar\nu=1/L`$. Then
> ```math
> \Omega=\Theta U\nu\ \cup\ \Theta U\bar\nu\ \cup\ U
> ```
> covers $`ℂ`$, and $`|\Omega|\le(2|\Theta|+1)|U|`$.
...
and $`\psi_n(x+iy)=x+y\,r_n`$ is a ring homomorphism $`ℤ[i]\toℤ/5^n`$ with kernel
$`P^nℤ[i]`$. Moreover
...
On $`ℤ[i]/P^n`$, multiplication by $`\theta_{13}`$ is multiplication by
...
$`5^k`$ for every $`k`$. The same holds on $`ℤ[i]/\bar P^{\,k}`$, where the multiplier is
...
The *label* of $`x\inℂ`$ is $`\Lambda(x)=\psi_n(\lfloor x/g\rfloor)\inℤ/5^n`$, with the
floor taken coordinatewise. For $`j\le\alpha`$ and $`t\le\beta`$ one has
$`\theta_5^{j}\theta_{13}^{t}\,g\,m=G_m/P^{n-j}`$ with $`G_m\inℤ[i]`$. Together with (a) this
gives an explicit unit $`X_j\in(ℤ/5^n)^\times`$ with
...
$`W_t\inℤ[i]`$ is prime to $`\bar P`$. Modulo $`\bar P^{\,n+4}`$ the $`W_t`$ are $`W_0`$ times the
...
$`\lambda`$ modulo the lattice $`\bar P^{\alpha}\bar Q^{\beta}ℤ[i]=gP^nℤ[i]`$. That
...
$`\lambda\inℤ/5^n`$ is the label of a difference of two orbit points.

**(d) Counting.** We have $`\lfloor(x-x')/g\rfloor=\lfloor x/g\rfloor-\lfloor x'/g\rfloor-d`$ with
$`d\in\{0,1,i,1+i\}`$. Hence
$`ℤ/5^n\subseteq Y-Y-\psi_n\{0,1,i,1+i\}`$ for $`Y=\Lambda(Z)`$, and so $`5^n\le4|Y|^2`$.
...
- $`y=\tilde Xz`$ runs over a set $`𝒴`$ of at least $`5^f`$ residues, distinct modulo $`5^\ell`$.

**(f) The discrete core.** This replaces the entropy input of
Bourgain–Lindenstrauss–Michel–Venkatesh. Let $`S=4\cdot5^{\ell-1}`$, and let $`N\inℕ`$
satisfy $`N\varepsilon^2\ge2`$ and $`25N^3<4|𝒴|`$. Suppose that every point
$`\xi_{t,y}=\tilde b^{\,t}(\gamma+y/5^\ell)`$ with $`t<S`$ and $`y\in𝒴`$ is at distance at
least $`\varepsilon/2`$ from $`ℤ`$.
...
```math
\sum_{t,y}F(\xi_{t,y})=\sum_{a,a'\lt N}\ \sum_{t\lt S}\sigma_t(a-a'),\qquad \sigma_t(m)=\sum_{y\in𝒴}e(m\,\xi_{t,y}).
```

The diagonal $`a=a'`$ contributes $`NS|𝒴|`$. Now let $`0<|m|<N`$. The value
$`|\sigma_t(m)|`$ does not depend on $`\gamma`$, and the map $`t\mapsto\tilde b^{\,t}\bmod 5^\ell`$
is injective on $`[0,S)`$. Hence

```math
\sum_{t\lt S}|\sigma_t(m)|^2\le\sum_{u\bmod 5^\ell}\Bigl|\sum_{y}e(muy/5^\ell)\Bigr|^2=5^\ell\,\#\{(y,y'):5^\ell\mid m(y-y')\}\le5^\ell|𝒴|\,|m|.
```

For the last step: the residues $`y'`$ with $`5^\ell\mid m(y-y')`$ are pairwise at least
$`5^\ell/|m|`$ apart. By Cauchy–Schwarz, $`\bigl|\sum_t\sigma_t(m)\bigr|\le5^\ell\sqrt{|𝒴||m|}`$.

Hence $`NS|𝒴|-N^2\,5^\ell\sqrt{|𝒴|N}\le S|𝒴|N/2`$. This forces
$`|𝒴|\le\frac{25}4N^3`$, a contradiction. So some $`\xi_{t,y}`$ lies within $`\varepsilon/2`$
of an integer.
