...
$$S_u(\varepsilon)=\{z\in\mathbb C:\ \|\operatorname{Re}(uz)\|<\varepsilon\}.$$
...
*Proof sketch.* The function $g(\varphi)=\operatorname{Re}(e^{i\varphi}e)$ has a zero
...
$q\,e^{i\varphi/2}$ with $q\approx128R/\varepsilon$. This changes $\operatorname{Re}(ue)$ by at most
...
$\|\operatorname{Re}(ue)\|<\varepsilon$. Since $uk\in\mathbb Z[i]$, we get
$\|\operatorname{Re}(uz)\|=\|\operatorname{Re}(ue)\|<\varepsilon$. This contradicts $u\in\Omega$.
$\square$
...
**(a) Residues and orders.** For $n\ge1$ the integer $\operatorname{Re}(P^n)$ is prime to $5$.
Put $r_n\equiv\operatorname{Im}(P^n)/\operatorname{Re}(P^n)\pmod{5^n}$. Then $r_n^2\equiv-1$,
and $\psi_n(x+iy)=x+y\,r_n$ is a ring homomorphism $\mathbb Z[i]\to\mathbb Z/5^n$ with kernel
$P^n\mathbb Z[i]$. Moreover
$\operatorname{Re}(z\bar P^n)\equiv\operatorname{Re}(P^n)\,\psi_n(z)\pmod{5^n}$, so

$$\operatorname{Re}(z/P^n)\equiv\operatorname{Re}(P^n)\,\psi_n(z)/5^n\pmod 1.$$
...
$$\bigl\|\operatorname{Re}(\theta_5^{j}\theta_{13}^{t}x)-\operatorname{val}(X_jb^t\Lambda(x))/5^{\,n-j}\bigr\|<\sqrt2\,|g|.$$
...
The diagonal $a=a'$ contributes $NS|\mathcal Y|$. For $0<|m|<N$:

- $|\sigma_t(m)|$ does not depend on $\gamma$.
- The map $t\mapsto\tilde b^{\,t}\bmod 5^\ell$ is injective on $[0,S)$, so
  ...
  $5^\ell/|m|$ apart.
- By Cauchy–Schwarz, $\bigl|\sum_t\sigma_t(m)\bigr|\le5^\ell\sqrt{|\mathcal Y||m|}$.
...
$\|\operatorname{Re}(\theta_5^{\,n-s}\theta_{13}^{\,t}x)\|<\varepsilon/2+\sqrt2|g|<\varepsilon$, so
